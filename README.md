...
Каждый экран приложения — отдельный Gradle-модуль. Внутри: Compose UI, ViewModel и UseCase/Interactor.
...
:feature:profile/
    ...
    ui/
        ProfileScreen.kt
        ProfileViewModel.kt
    domain/                     ← UseCase, которые нужны только этому экрану
...
**ViewModel не обращается к репозиторию напрямую — только через UseCase**, даже если UseCase просто прокидывает вызов в репозиторий. Так ViewModel видит только операции, нужные экрану, а не весь API репозитория.
...
UseCase нужен всегда, даже без собственной логики (см. раздел «Feature-слой»). Domain-модуль же — только по условиям выше: UseCase одного экрана живёт в пакете `domain/` этого feature-модуля.
...
:core:database/
    ...
    StayaDatabase.kt           ← internal
    DatabaseCleaner.kt         ← internal, очистка всех таблиц в начале сессии
    di/DatabaseModule.kt       ← @Singleton БД, @Provides для каждого DAO, @IntoSet DatabaseCleaner
    pet/                       ← пакет на каждый домен
        Pet*Entity.kt
        PetDao.kt
```

- `StayaDatabase` — `internal`: снаружи модуля доступны только DAO, получить базу целиком нельзя.
...
БД содержит данные конкретного пользователя с конкретного стенда. Сбросом данных прошлой сессии занимаются участники начала сессии — реализации `SessionStartListener` из `:core:common`, зарегистрированные через `@IntoSet`. `AuthRepository` не знает, что они делают: при входе и подтверждении email он вызывает у всех `onSessionStart()` и только потом сохраняет токены. База — один из таких участников: `DatabaseCleaner` очищает все таблицы.

Поэтому новая сессия всегда начинается с пустой базы, как бы ни завершилась прошлая: выходом, сбросом токенов сетевым слоем или смертью процесса. Смена стенда тоже проходит через новый вход.

Данные прошлой сессии лежат в базе до следующего входа.

Данные прошлой сессии лежат в базе до следующего входа. Когда добавим явный выход, добавим и отчистку.
...
|---|---|
| UseCase нужен одному экрану — с логикой или без | `domain/` пакет внутри feature-модуля |
